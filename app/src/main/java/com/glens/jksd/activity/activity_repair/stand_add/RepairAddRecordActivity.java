 ...
 * 录音条数最多3条，时长最长60s
 */
public class RepairAddRecordActivity extends BaseActivity implements View.OnClickListener, AdapterView.OnItemClickListener, RepairTowerAddView {
    public static final String TAG = RepairAddRecordActivity.class.getSimpleName();
    @Bind(R.id.tv_repair_pic_title)
    TextView mTvRepairPicTitle;
    @Bind(R.id.rb_repair_task_start)
    RadioButton mRbRepairTaskStart;
    @Bind(R.id.rb_repair_task_end)
    RadioButton mRbRepairTaskEnd;
    ...
    @Bind(R.id.tv_repair_pic)
    TextView tvRepairPic;
    @Bind(R.id.ry_record)
    ...
    private static final int REQUEST_CHECK_PIC = 116;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_repair_add_record);
    ...
    private void initView() {
        ...
        mRecordLayout = 1;
        soundRecordorId = PreferenceUtils.getString(this, RepairConstantUtils.USER_SAVE_CODE, "");
        mUserName = PreferenceUtils.getString(this, RepairConstantUtils.USER_SAVE_NAME, "");
        mTvCheckPerson.setText(mUserName);

        mRgRepairStandard.check(R.id.rb_repair_task_start);
        mRgRepairStandard.setOnCheckedChangeListener((group, checkedId) -> {
            switch (checkedId) {
                case R.id.rb_repair_task_start:
                    soundType = RepairConstantUtils.START_WORK;
                    break;
                case R.id.rb_repair_task_end:
                    soundType = RepairConstantUtils.DOWN_WORK;
                    break;
                default:
                    break;
            }
        });
        setTitleText(R.string.repair_record_add);
    ...
    private void initNetData() {
        ...
        mPresenter = new RepairGetMapPresenter(RepairAddRecordActivity.this);
        mPresenter.onCreate();
        mPresenter.BindPresentView(RepairAddRecordActivity.this);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        switch (parent.getId()) {
            case R.id.grid_view:
                ...
                ImagePickUtils.promptSelectImgDialog(event, position, this, mImageList, REQUEST_CHECK_PIC, 4, R.id.ry_record);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.iv_repair_record_delete:
                ...
                mPresenter.checkDeleteSound(RepairConstantUtils.REPAIR_RECORD_ITEM_ONE, mAudioPathMap, TAG,RepairConstantUtils.WORK_RECORD_TYPE);
                break;
            case R.id.iv_repair_record_delete_two:
                ...
                mPresenter.checkDeleteSound(RepairConstantUtils.REPAIR_RECORD_ITEM_TWO, mAudioPathMap, TAG,RepairConstantUtils.WORK_RECORD_TYPE);
                break;
            case R.id.iv_repair_record_delete_three:
                ...
                mPresenter.checkDeleteSound(RepairConstantUtils.REPAIR_RECORD_ITEM_THREE, mAudioPathMap, TAG,RepairConstantUtils.WORK_RECORD_TYPE);
                break;
            case R.id.bt_repair_record:
                isStartRecord(mRecordLayout);
                break;
            case R.id.repair_task_standard_ry:
                if (mPresenter.checkRecordData(this, mTaskCode, soundType, soundRecordorId, mEtRecordName.getText().toString().trim())) {
                    mPresenter.addSoundRecord();
                }
                break;
        }
    }
    ...
    private Runnable mUpAudio = () -> {
        mPresenter.upAudioData(mCurrentItem, TAG, mTaskCode, this,RepairConstantUtils.WORK_RECORD_TYPE);
    };
    ...
    public void onEventMainThread(BaseEvent event) {
        ...
        switch (event.getEventType()) {
            case RepairConstantUtils.NET_POST_DETECTION_IMAGE:
                ...
                mPresenter.upWorkRecordPhoto(PowerApplication.getmDataList().get(0).sourcePath, mTaskCode, "测试");
                break;
            ...
            case RepairConstantUtils.DELETE_IMAGE_SUCCESS:
                if (!TextUtils.isEmpty(picCode)) {
                    ...
                    mPresenter.deleteSoundPicture(picCode);
                }
                break;
    ...
    public HashMap<String, Object> getDataMap() {
        ...
        map.put("soundRecordorId", soundRecordorId);
        map.put("soundName", mEtRecordName.getText().toString().trim());
