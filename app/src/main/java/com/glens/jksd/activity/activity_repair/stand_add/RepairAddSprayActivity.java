 ...
 * 新增RTV喷涂
 */
public class RepairAddSprayActivity extends BaseActivity implements View.OnClickListener, AdapterView.OnItemClickListener, RepairTowerAddView {
    public static final String TAG = RepairAddSprayActivity.class.getSimpleName();
    @Bind(R.id.rg_spray_add)
    RadioGroup mRgSprayAdd;
    @Bind(R.id.repair_task_standard_text)
    TextView mRepairTaskStandardText;
    @Bind(R.id.repair_title_two)
    TextView repairTitleTwo;
    @Bind(R.id.repair_equipment)
    TextView repairEquipment;
    @Bind(R.id.repair_title_three)
    TextView repairTitleThree;
    @Bind(R.id.repair_power_level)
    TextView repairPowerLevel;
    @Bind(R.id.repair_title_four)
    TextView repairTitleFour;
    @Bind(R.id.repair_title_one)
    TextView repairTitleOne;
    @Bind(R.id.repair_loc)
    EditText repairLoc;
    @Bind(R.id.tv_repair_content_title)
    TextView tvRepairContentTitle;
    @Bind(R.id.et_repair_content)
    EditText etRepairContent;
    @Bind(R.id.rb_spray_before)
    RadioButton rbSprayBefore;
    @Bind(R.id.rb_spray_one)
    RadioButton rbSprayOne;
    @Bind(R.id.rb_spray_two)
    RadioButton rbSprayTwo;
    @Bind(R.id.repair_task_standard_ry)
    RelativeLayout repairTaskStandardRy;
    @Bind(R.id.spray_grid_view)
    MyGridView mSprayGridView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_repair_spary);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        switch (parent.getId()) {
            case R.id.spray_grid_view:
                BaseEvent event = new BaseEvent();
                event.setEventType(RepairConstantUtils.NET_POST_DETECTION_IMAGE);
                ImagePickUtils.promptSelectImgDialog(event, position, RepairAddSprayActivity.this, mImageList, REQUEST_CHECK_PIC, 4, R.id.fy_spray);
    ...
    private void initImageData() {
        mSprayGridView.setHaveScrollbar(false);
        mSprayGridView.setOnItemClickListener(this);
        mSprayGridView.setOnItemLongClickListener((parent, view, position, id) -> {
            if (isShowDelete) {
                isShowDelete = false;
            } else {
                isShowDelete = true;
        ...
        mSprayGridView.setAdapter(mImageListAdapter);
    }
    ...
    private void initNetData() {
        ...
        mPresenter = new RepairGetMapPresenter(RepairAddSprayActivity.this);
        mPresenter.onCreate();
        mPresenter.BindPresentView(RepairAddSprayActivity.this);
    ...
    private void initView() {
        setTitleText(R.string.repair_spray_add);
        getLlBasetitleBack().setOnClickListener(this);
        mRepairTaskStandardText.setText(R.string.repair_check_submit);
        mRgSprayAdd.check(R.id.rb_spray_before);
            ...
            case RepairConstantUtils.SELECT_EQUIPMENT:
                if (resultCode == RepairConstantUtils.SELECT_EQUIPMENT_RESULT) {
                    if (data != null) {
                        ...
                        repairEquipment.setText(String.valueOf(lineName + lineNumber + "杆"));
                        repairPowerLevel.setText(String.valueOf(powerLevel + "KV"));
                    }
                }
                break;
        }
    ...
    public HashMap<String, Object> getDataMap() {
        ...
        map.put("loc",repairLoc.getText().toString().trim());
    ...
    public void onEventMainThread(BaseEvent event) {
        ...
        switch (event.getEventType()) {
            case RepairConstantUtils.NET_POST_DETECTION_IMAGE:
                ...
                mPresenter.upSprayPhoto(PowerApplication.getmDataList().get(0).sourcePath, mTaskCode,RepairConstantUtils.SPRAY_ONE, "测试");
                break;
            ...
            case RepairConstantUtils.DELETE_IMAGE_SUCCESS:
                if (!TextUtils.isEmpty(picCode)) {
                    ...
                    mPresenter.deleteSprayPhoto(picCode);
                }
                break;
    ...
    @OnClick({R.id.repair_equipment, R.id.repair_task_standard_ry})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.repair_equipment:
                mPresenter.changeToSelect(mTaskCode, RepairConstantUtils.LINE_SPRAY, RepairAddSprayActivity.this);
                break;
            case R.id.repair_task_standard_ry:
                if (mPresenter.checkSprayDataSet(this, lineName, lineNumber, lineNumber,selectText, repairLoc)) {
                    mPresenter.addSpray();
                }
                break;
        }
