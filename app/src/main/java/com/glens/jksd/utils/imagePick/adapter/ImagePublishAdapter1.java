 ...
 */
public class ImagePublishAdapter1 extends BaseAdapter {
    ...
    public ImagePublishAdapter1(Context context, List<ImageItem> dataList, int maxSize) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (isShowAddItem(position)) {
            imageIv.setImageResource(R.drawable.repair_add);
...
//                GlideUtils.loadImageViewOrError(mContext,StringUtil.getPath(item), imageIv, R.drawable.img_default);
                ...
                if (item.isPdf) {
                    imageIv.setImageResource(R.drawable.pdf);
                } else {
//                    GlideUtils.loadImageViewLoadingSize(mContext,StringUtil.getPath(item),R.dimen.space_45,R.dimen.space_45 ,imageIv, R.drawable.img_default);
                    Glide.with(mContext).load(StringUtil.getPath(item)).override(R.dimen.space_45, R.dimen.space_45).error(R.drawable.img_default).into(imageIv);
                }
            }
        }
        if (!isShowAddItem(position)) {
        ...
        if (del.getVisibility() == View.VISIBLE) {
            del.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    delPos = position;
//                    EventBus.getDefault().post(Event.DELETE_IMAGE);
